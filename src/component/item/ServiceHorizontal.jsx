...
export default class ServiceHorizontal extends Component {
	...
	getTaxonomyTagsContent() {
		if (this.props.taxonomy
			...
			&& this.props.info.taxonomy_tags) {
			const taxonomyValues = this.props.taxonomy.values
			...
			return <div className="card-tags">
				{taxonomyValues.map((v) => <Chip
					key={v.name}
					label={v.name}
					url={getMainAppURL() + "search?taxonomy_value=" + v.id}
				/>)}
			</div>;
		}
	...
	getCompanyTagsContent() {
		if (this.props.info.company_tags && this.props.companies) {
			return <div className="card-tags">
	...
	render() {
		return this.props.info.link
			...
			: <a
				href={getMainAppURL() + "service/" + this.props.info.handle}
				className="ServiceHorizontal-link">
				{this.getBoxContent()}
			</a>;
	}
